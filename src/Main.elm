module Main exposing (Model, Msg(..), init, main, subscriptions, update, view, viewAll)
...
import Port exposing (setStorage)
...
update msg model =
    case msg of
        ...
        PressedClearSetup ->
            ( { model | grid = Matrix.initialize 9 9 clearCell }, Cmd.none )
...
restoreModel string =
    case Decode.decodeString decodeModel string of
        ...
        Err _ ->
            { viewMode = SetUp
            , grid = Matrix.initialize 9 9 clearCell
            }
    ...
    column [ centerX, width fill ]
        [ row topRowAttributes
            ...
            , Input.button buttonAttr
                { onPress = Just PressedClearSetup
                , label = text "Clear"
                }
            , Input.button buttonAttr
